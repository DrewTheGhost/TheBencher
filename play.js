    ...
    async fn(message, suffix, bot, db) {
        ...
        if(!ytdl.validateURL(suffix) && !ytpl.validateID(suffix)) {
            ...
            https.get(`https://www.googleapis.com/youtube/v3/search?part=snippet&fields=items(id,snippet(title))&maxResults=5&q=${suffix}&key=${config.youtubeAPIKey}`, (res) => {
                ...
                if(statusCode != 200) {
                    return message.channel.send(`Shit brokey, returned status code ${statusCode}`)
                }
                res.on("data", d => {
                    chunk += d.toString();
                });
                ...
                return message.channel.send("Can't seem to pull info from that. Did you actually put in a youtube link or proper search request, asshole?");
            })
        }
        ...
        */
        setTimeout(async () => {
            if(ytdl.validateURL(suffix)) {
                let errored = false;
                requestInfo = await ytdl.getInfo(suffix).catch(err => {
                    errored = true;
                    console.error(err)
                })
                if(errored) {
                    return message.channel.send("SHIT BROKEY!!")
                }
            ...
            if(bot.voice.adapters.get(message.guild.id) == undefined) {
                ...
                db.client.query("SELECT * FROM queue ORDER BY id ASC LIMIT 1;", (err, result) => {
                    ...
                    var stream = ytdl(result.rows[0].url, {filter: "audioonly", quality: "highestaudio"}).pipe(fs.createWriteStream("./video.mp4"));
                    console.debug("Stream created using ytdl");
                    console.debug(stream.listenerCount("finish"))

                    if(stream.listenerCount("finish") <= 1) { 
                        stream.on("finish", () => {
                            let resource = createAudioResource("video.mp4", { inputType: StreamType.Arbitrary })
